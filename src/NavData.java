...
public class NavData
{
    ...
    public static enum CtrlState
    {
        ...
        public static CtrlState fromInt(int v) throws Exception
        {
            switch(v)
            {
            case 0:
            ...
            default:
                throw new Exception("Invalid control state " + v);
            }
        }
    }
    ...
    public static NavData createFromData(byte[] buf) throws Exception
    {
        System.out.println("Parsing navdata len=" + buf.length);
        ...
        if(header != 0x55667788)
            throw new Exception("Error parsing NavData");
        offset += 4;
        ...
        while(offset < buf.length)
        {
            ...
            if(option_len == 0)
                throw new Exception("Zero-len option with tag " + option_tag);
            ...
            {
                System.out.println("Skipping unknown NavData option with tag=" + option_tag);
            }
            offset = offset + option_len - 4;
        }

        // TODO: calculate checksum
        System.out.println("Got Nav data. mode " + data.mode);

        return data;
    }

    private static List<VisionTag> parseVisionTags(NavData data, byte[] buf, int offset) throws Exception
    {
        int nb_detected = byteArrayToInt(buf, offset);
        offset += 4;

        if(nb_detected!=0)
            System.out.println("" + nb_detected + " vision tags detected");
        ...
        for(int i = 0; i < nb_detected; i++)
        {
            ...
            VisionTag vt = new VisionTag(VisionTag.VisionTagType.fromInt(type), new Point(xc, yc), new Dimension(width, height),
                    dist);
            System.out.println("Vision#" + i + " " + vt.toString());
            res.add(vt);
        }
    ...
    private static void parseDemoNavData(NavData data, byte[] buf, int offset) throws Exception
    {
    	System.out.println(buf.length+" "+offset);
        data.ctrl_state = CtrlState.fromInt(byteArrayToInt(buf, offset) >> 16);
        System.out.println("Ctrl State " + data.ctrl_state);
    ...
    public static StringBuffer toStringBuffer(NavData data)
    {
    ...
    {
        System.out.println("State: " + toStringBuffer(data).toString());
    }
    public static String toString(NavData data)
    {
        return toStringBuffer(data).toString();
    }
